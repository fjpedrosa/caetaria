...
export interface VerifyPhoneNumberDependencies {
  readonly onboardingRepository: OnboardingRepository;
  readonly whatsappService: WhatsAppService;
}

/**
 * Factory function to create verifyPhoneNumber use case
 * @param deps Dependencies required for the use case
 * @returns Object with startVerification and completeVerification functions
 */
export const createVerifyPhoneNumberUseCase = (deps: VerifyPhoneNumberDependencies) => {
  const { onboardingRepository, whatsappService } = deps;

  return {
    startVerification: async (command: StartPhoneVerificationCommand): Promise<Result<OnboardingSession, Error>> => {
      try {
        ...
        const verificationResult = await whatsappService.sendVerificationCode(
        ...
        const saveResult = await onboardingRepository.update(updatedSession);
    ...
    completeVerification: async (command: CompletePhoneVerificationCommand): Promise<Result<OnboardingSession, Error>> => {
      try {
        // Find the onboarding session
        const sessionResult = await onboardingRepository.findById(
          command.sessionId as OnboardingSessionId
...
/**
 * @deprecated Use createVerifyPhoneNumberUseCase factory function instead
...
export class VerifyPhoneNumberUseCase {
  ...
  async startVerification(command: StartPhoneVerificationCommand): Promise<Result<OnboardingSession, Error>> {
    const useCase = createVerifyPhoneNumberUseCase({
      onboardingRepository: this.onboardingRepository,
      whatsappService: this.whatsappService
    });
    return useCase.startVerification(command);
  }
  ...
  async completeVerification(command: CompletePhoneVerificationCommand): Promise<Result<OnboardingSession, Error>> {
    ...
    return useCase.completeVerification(command);
  }
}
