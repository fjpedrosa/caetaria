...
export interface SubmitBusinessInfoDependencies {
  readonly onboardingRepository: OnboardingRepository;
}

/**
 * Factory function to create submitBusinessInfo use case
 * @param deps Dependencies required for the use case
 * @returns Object with execute function
 */
export const createSubmitBusinessInfoUseCase = (deps: SubmitBusinessInfoDependencies) => {
  const { onboardingRepository } = deps;

  return {
    execute: async (command: SubmitBusinessInfoCommand): Promise<Result<OnboardingSession, Error>> => {
      try {
...
/**
 * @deprecated Use createSubmitBusinessInfoUseCase factory function instead
...
export class SubmitBusinessInfoUseCase {
  ...
  async execute(command: SubmitBusinessInfoCommand): Promise<Result<OnboardingSession, Error>> {
    const useCase = createSubmitBusinessInfoUseCase({ onboardingRepository: this.onboardingRepository });
