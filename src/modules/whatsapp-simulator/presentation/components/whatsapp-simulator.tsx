...
import type {
  WhatsAppSimulatorPresentationalProps
} from '../../domain/types';
...
export const WhatsAppSimulator = React.memo<WhatsAppSimulatorPresentationalProps>(
  function WhatsAppSimulator({
    ...
    const messages = conversationFlow?.state?.messages || [];
    const currentMessageIndex = conversationFlow?.state?.currentMessageIndex || 0;
    const isTyping = typingIndicator?.state?.isAnyoneTyping || false;
    const activeTypingUsers = typingIndicator?.state?.activeTypingUsers || [];
    ...
    console.log(`[WhatsAppSimulator UI ${instanceId}] Render con estado actualizado:`, {
      ...
      lastVisibleMessage: lastVisibleMessage.substring(0, 50) + (lastVisibleMessage.length > 50 ? '...' : ''),
      isTyping,
      conversationStatus: conversation?.status
    });

    // Debug: Track when messages become visible
    React.useEffect(() => {
      if (messages.length > 0 && currentMessageIndex >= 0) {
        const currentMessage = messages[currentMessageIndex];
        if (currentMessage) {
          const messageText = getMessageDisplayText(currentMessage);
          console.log(`🎬 [WhatsAppSimulator] Mensaje ${currentMessageIndex + 1}/${messages.length} ahora visible:`, {
    ...
    // Main simulator UI - Always render the frame, with progressive content
    return (
    <motion.div
              ...
              <div className="relative bg-white dark:bg-gray-900 rounded-[2.7rem] overflow-hidden" style={{ aspectRatio: '390/844' }}>
                ...
                <div className="h-full flex flex-col">
                  ...
                  <div className="flex-1 bg-[#e5ddd5] bg-[url('data:image/svg+xml,%3Csvg%20width%3D%2260%22%20height%3D%2260%22%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%3E%3Cdefs%3E%3Cpattern%20id%3D%22whatsapp-bg%22%20patternUnits%3D%22userSpaceOnUse%22%20width%3D%2260%22%20height%3D%2260%22%3E%3Cpath%20d%3D%22M0%2030h60v30H0z%22%20fill%3D%22%23e5ddd5%22%20opacity%3D%22.8%22%2F%3E%3Cpath%20d%3D%22M0%200h60v30H0z%22%20fill%3D%22%23d9d0c7%22%20opacity%3D%22.8%22%2F%3E%3C%2Fpattern%3E%3C%2Fdefs%3E%3Crect%20width%3D%22100%25%22%20height%3D%22100%25%22%20fill%3D%22url(%23whatsapp-bg)%22%2F%3E%3C%2Fsvg%3E')] p-4 space-y-3 overflow-hidden">
                    {!isContentReady ? (
                      // Loading state inside the chat area
                      <div className="flex items-center justify-center h-full">
                        <div className="text-center">
                          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-green-500 mx-auto mb-2"></div>
                          <p className="text-xs text-gray-600">Cargando conversación...</p>
                        </div>
                      </div>
                    ) : (
                      <>
                        ...
                        <AnimatePresence>
                          {isTyping && (
                            <motion.div
                              className={`flex ${
                                activeTypingUsers.includes('business') ? 'justify-end' : 'justify-start'
                              }`}
                              initial={{ opacity: 0, scale: 0.95 }}
                              animate={{ opacity: 1, scale: 1 }}
                              exit={{ opacity: 0, scale: 0.95 }}
