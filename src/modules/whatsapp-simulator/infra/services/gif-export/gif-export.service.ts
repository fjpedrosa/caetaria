...
export class GifExportServiceImpl implements GifExportService {
  ...
  async exportConversation(
    ...
    if (this.isExporting) {
      throw this.createError('export_in_progress', 'Another export is already in progress');
    }
    ...
    try {
      ...
      const optimizedOptions = this.optimizeExportOptions(options, element);
      ...
      if (this.currentOperation.signal.aborted) {
        throw this.createError('export_cancelled', 'Export was cancelled');
      }
      ...
      const blob = await this.generateGif(frames, optimizedOptions, (progress) => {
        callbacks?.onProgress?.({
          stage: 'encoding',
      ...
      const result: GifExportResult = {
        ...
        metrics: {
          ...
          compressionRatio: this.calculateCompressionRatio(frames, blob)
        },
        metadata: {
          createdAt: new Date(),
      ...
      const gifError = this.handleError(error);
  ...
  async captureFrames(
    ...
    try {
      ...
      if (frames.length === 0) {
        throw this.createError('capture_failed', 'No frames were captured successfully');
      }
  ...
  async generateGif(
    ...
    return new Promise((resolve, reject) => {
      if (frames.length === 0) {
        reject(this.createError('encoding_failed', 'No frames to encode'));
        return;
      }
      ...
      gif.on('progress', (progress: number) => {
        onProgress?.({
          ...
          totalFrames: frames.length,
          processingSpeed: this.calculateProcessingSpeed(progress, frames.length)
        });
      });
      ...
      gif.on('error', (error: Error) => {
        reject(this.createError('encoding_failed', `GIF encoding failed: ${error.message}`, error));
      });
      ...
      try {
        ...
        reject(this.createError('encoding_failed', `Failed to add frames: ${error}`, error));
      }
    });
  }
  ...
  private validateExportOptions(options: ExportOptions): void {
    if (options.quality < 0.1 || options.quality > 1.0) {
      throw this.createError('invalid_options', 'Quality must be between 0.1 and 1.0');
    }
    if (options.frameRate < 5 || options.frameRate > 30) {
      throw this.createError('invalid_options', 'Frame rate must be between 5 and 30 fps');
    }
    if (options.duration < 5 || options.duration > 60) {
      throw this.createError('invalid_options', 'Duration must be between 5 and 60 seconds');
    }
    if (options.scale < 0.5 || options.scale > 2.0) {
      throw this.createError('invalid_options', 'Scale must be between 0.5 and 2.0');
    }
  }
  ...
  private startMemoryMonitoring(): void {
    if ('memory' in performance) {
      ...
      this.memoryMonitor.currentUsage = memory.usedJSHeapSize / (1024 * 1024); // MB
      this.memoryMonitor.peakUsage = Math.max(this.memoryMonitor.peakUsage, this.memoryMonitor.currentUsage);
      this.memoryMonitor.nearLimit = this.memoryMonitor.currentUsage > (this.memoryMonitor.limit * 0.8);
    }
  }
  ...
  private updateMemoryUsage(): void {
    if ('memory' in performance) {
      const memory = (performance as any).memory;
      this.memoryMonitor.currentUsage = memory.usedJSHeapSize / (1024 * 1024);
      this.memoryMonitor.peakUsage = Math.max(this.memoryMonitor.peakUsage, this.memoryMonitor.currentUsage);
      this.memoryMonitor.nearLimit = this.memoryMonitor.currentUsage > (this.memoryMonitor.limit * 0.8);
    }
  }

  private calculateCompressionRatio(frames: Frame[], blob: Blob): number {
  ...
  private calculateProcessingSpeed(progress: number, totalFrames: number): number {
  ...
  private handleError(error: any): GifExportError {
 ...
 * Singleton instance
 */
export const gifExportService = new GifExportServiceImpl();